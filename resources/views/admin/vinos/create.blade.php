...
@section('contenido')
    <div class="header-admin">
        <h1>Agregar Nuevo Vino</h1>
        <a href="{{ route('vinos.index') }}" class="btn-linea"><i class="fas fa-arrow-left"></i> Volver</a>
    </div>
    ...
    <section class="form-container">
        <form action="{{ route('vinos.store') }}" method="POST">
            ...
            <fieldset class="input-group">
                ...
                <input type="text" id="nombre" name="nombre" placeholder="Ej: Malbec Reserva" required>
            </fieldset>

            <fieldset class="input-group">
                <label for="descripcion">Descripción</label>
                <textarea id="descripcion" name="descripcion" rows="4" placeholder="Describe las notas, maridaje, etc." required></textarea>
            </fieldset>

            <fieldset class="input-group">
                <label for="precio">Precio por Botella ($)</label>
                <input type="number" step="0.01" id="precio" name="precio" placeholder="Ej: 750.00" required>
            </fieldset>

            <button type="submit" class="btn-dorado">Guardar Vino</button>
        </form>
    </section>
@endsection
