...
@section('contenido')
    <div class="header-admin">
        <h1>Editar Vino</h1>
        <a href="{{ route('vinos.index') }}" class="btn-linea"><i class="fas fa-arrow-left"></i> Cancelar</a>
    </div>
    ...
    <section class="form-container">
        <form action="{{ route('vinos.update', $vino->id) }}" method="POST">
            ...
            <fieldset class="input-group">
                ...
                <textarea id="descripcion" name="descripcion" rows="4" required>{{ $vino->descripcion }}</textarea>
            </fieldset>

            <fieldset class="input-group">
                <label for="precio">Precio por Botella ($)</label>
                <input type="number" step="0.01" id="precio" name="precio" value="{{ $vino->precio }}" required>
            </fieldset>

            <button type="submit" class="btn-dorado">Actualizar Vino</button>
        </form>
    </section>
@endsection
